// Auto-generated by scripts/build-bundles.js @ 2025-12-02T17:36:09.130Z
(function () {
...
const keepAliveMutex = (() => {
  ...
  async function tryQwenPipeline(promptText = '', lifecycle = {}) {
    ...
    try {
      const pipeline = new window.UnifiedAnswerPipeline('qwen', {
        expectedLength: pipelineExpectedLength(promptText)
      });
  ...
  async function withSmartScroll(asyncOperation, options = {}) {
    if (qwenScrollCoordinator) {
      return qwenScrollCoordinator.run(asyncOperation, options);
    }
  ...
  async function sendComposer(input) {
    ...
    // Затем ищем кнопку отправки (проверенные Qwen селекторы)
    const sendBtn = document.querySelector(
      [
        'button.ant-btn-primary:not([disabled])',
        'button:has(svg.icon-send)',
  ...
  async function waitForQwenReply(prompt, timeout = 150000) {
    const start = Date.now();
    const notBefore = start - 2000;
    const expectedFingerprint = computePromptFingerprint(prompt || '');
    const initialAssistantCount = getAssistantMessages().length;
  ...
  function sendResult(text, ok = true, context = { isEvaluator: false }, error = null) {
        ...
        return await withSmartScroll(async () => {
          ...
          await tryQwenPipeline(prompt, {
            ...
            stop: async ({ answer }) => {
              ...
              sendResult(cleaned, true, context);
  ...
  const onRuntimeMessage = (msg, _sender, sendResponse) => {
    try {
      ...
      if (msg?.type === 'GET_ANSWER' || msg?.type === 'GET_FINAL_ANSWER') {
        currentRequestContext = { isEvaluator: Boolean(msg.isEvaluator) };
